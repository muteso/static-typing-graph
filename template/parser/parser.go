...
// as TemplateHolder-struct; if any error occurs doesn't interrupt parsing
...
func (bp bProperty) toActual(c *context) {
	...
	typs, err := template.ToDataType(bp.BufType)
	if err != nil {
		e := parseError{
...
func (br bRestrictions) toActual(c *context) {
	...
	for i, v := range br.BufKeyRegexpRestr {
		r, err := template.MutateRestr(typs, template.TKeyRegExp, v)
		if err != nil {
			e := parseError{
